...
import { ArrowLeftCircle, LogIn, Star, Trophy, Trash2 } from 'lucide-react';
...
export default function CompletedPage() {
  ...
  const { data: completedItems = [], isLoading, error } = useLibrary('completed', {
    enabled: isAuthenticated,
  });

  const { data: stats } = useLibraryStats({ enabled: isAuthenticated });
  ...
  const redirectToSignIn = () => {
    router.push('/auth/signin?callbackUrl=%2Fcompleted');
  };

  const handleMoveToBacklog = async (item: LibraryItem) => {
    setActionError(null);
  ...
  return (
    <div className="mx-auto w-full max-w-350 px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      <div className="apple-surface apple-card-shadow rounded-[32px] border border-white/10 p-8 sm:p-10">
        <p className="apple-caption mb-2">Progress</p>
        <h1 className="apple-title text-4xl font-semibold sm:text-5xl">Completed Games</h1>
        <p className="mt-3 max-w-2xl text-muted-foreground">
          Celebrate finished games and keep a lasting record of your all-time favorites.
        </p>

        <div className="mt-7 flex flex-wrap gap-2">
          <Badge variant="outline" className="rounded-full border-white/20 px-3 py-1">
            <Trophy className="mr-1.5 h-3.5 w-3.5" />
            {stats?.completedGames ?? sortedCompleted.length} completed
          </Badge>
          <Badge variant="outline" className="rounded-full border-white/20 px-3 py-1">
      ...
      {isSessionLoading ? (
        <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={`completed-session-loading-${index}`} className="apple-surface animate-pulse rounded-3xl border border-white/10 p-4">
      ...
      {!isSessionLoading && requiresSignIn ? (
        <div className="apple-surface mt-8 rounded-3xl border border-white/10 p-10 text-center">
          <h2 className="text-2xl font-semibold">Sign in to view completed games</h2>
          <p className="mt-2 text-muted-foreground">
            Completion history is saved to your account and available on any device.
          </p>
      ...
      {!isSessionLoading && !requiresSignIn && isLoading ? (
        <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={`completed-loading-${index}`} className="apple-surface animate-pulse rounded-3xl border border-white/10 p-4">
              <div className="mb-4 h-36 rounded-2xl bg-muted" />
              <div className="h-4 w-2/3 rounded bg-muted" />
              <div className="mt-2 h-3 w-1/2 rounded bg-muted" />
            </div>
      ...
      {!isSessionLoading && !requiresSignIn && !isLoading && error ? (
        <div className="apple-surface mt-8 rounded-3xl border border-rose-400/30 bg-rose-500/10 p-6 text-sm text-rose-200">
          {error instanceof Error ? error.message : 'Failed to load completed games.'}
        </div>
      ) : null}

      {!isSessionLoading && !requiresSignIn && !isLoading && !error && sortedCompleted.length === 0 ? (
        <div className="apple-surface mt-8 rounded-3xl border border-white/10 p-10 text-center">
          <p className="text-lg text-muted-foreground">
            Finish a game from your backlog to start building this showcase.
          </p>
        </div>
      ) : null}
            ...
            return (
              <article key={item.id} className="apple-surface rounded-3xl border border-white/10 p-4">
                <Link href={`/games/${item.gameId}`} className="block">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Completed {completedLabel}</p>
                </Link>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  >
                    ...
                    <span className="sr-only">Remove from completed list</span>
                  </Button>
                </div>
              </article>
            );
